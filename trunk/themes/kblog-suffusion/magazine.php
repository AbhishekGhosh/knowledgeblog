...
function suffusion_show_mag_excerpts($queries, $total) {
	global $post, $suf_mag_excerpts_per_row, $suf_mag_total_excerpts;
	$ctr = 0;
	$ret = "";
...
function suffusion_show_mag_single_excerpt() {
	...
	if (function_exists('get_the_post_thumbnail') && has_post_thumbnail($post->ID)) {
		$image_link = suffusion_get_post_thumbnail('excerpt');
	}
	else {
