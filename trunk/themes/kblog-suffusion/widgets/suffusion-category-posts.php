...
class Suffusion_Category_Posts extends WP_Widget {
	function Suffusion_Category_Posts() {
		$widget_ops = array('classname' => 'widget-suf-cat-posts',
			'description' => __("A widget for displaying posts in a category. You can use this to selectively display posts.", "suf_theme"));

		$control_ops = array('width' => 600);
		$this->WP_Widget("suf-cat-posts", __("Category Posts", "suf_theme"), $widget_ops, $control_ops);
	}
...
<?php
		_e("<p>This widget lets you display the latest posts from a category.</p>", "suf_theme");
?>

		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e('Title:', 'suf_theme'); ?></label>
			<input id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo $instance['title']; ?>" class="widefat" />
			<i><?php _e("This will default to the name of the selected category if left blank", "suf_theme"); ?></i>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id('selected_category'); ?>"><?php _e('Select category to show:', 'suf_theme'); ?></label>
<?php
...
?>
			<i><?php _e("Posts from the selected category will show in the widget", "suf_theme"); ?></i>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'number_of_posts' ); ?>"><?php _e('Number of posts to display:', 'suf_theme'); ?></label>
...
?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'post_style' ); ?>"><?php _e('Post display style:', 'suf_theme'); ?></label>
			<select id="<?php echo $this->get_field_id( 'post_style' ); ?>" name="<?php echo $this->get_field_name( 'post_style' ); ?>" class='widefat'>
<?php
		$post_styles = array('sidebar' => __('Underline upon hover (sidebar-style)', 'suf_theme'),
				'magazine' => __('Box upon hover (magazine-style)', 'suf_theme'),
			);
...
?>
			...
			<i><?php _e("You can choose to display an icon for the category based on the Category Icons plugin, or a custom image", "suf_theme"); ?></i>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'all_posts_text' ); ?>"><?php _e('Text for "All Posts":', 'suf_theme'); ?></label>
			...
			<i><?php _e("The text you enter here will be the displayed in the button for \"All Posts\". If you leave this field blank the button will not be shown.", "suf_theme"); ?></i>
		</p>
	</div>
	<div style='float: right; width: 48%;'>
		<p>
			<label for="<?php echo $this->get_field_id( 'icon_type' ); ?>"><?php _e('Category Icon:', 'suf_theme'); ?></label>
			<select id="<?php echo $this->get_field_id( 'icon_type' ); ?>" name="<?php echo $this->get_field_name( 'icon_type' ); ?>" class='widefat'>
<?php
		$icon_types = array('none' => __('No category icon', 'suf_theme'),
				'plugin' => __('Use Category Icons Plugin', 'suf_theme'),
				'custom' => __('Use custom image', 'suf_theme'),
			);
...
?>
		...
		<p>
			<label for="<?php echo $this->get_field_id('cat_icon_url'); ?>"><?php _e('Category icon custom image link:', 'suf_theme'); ?></label>
			...
			<i><?php _e("If you have chosen to use a custom image in the previous option, please enter the complete URL of the category icon here, including http://", "suf_theme"); ?></i>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'icon_height' ); ?>"><?php _e('Set the height for the category icon:', 'suf_theme'); ?></label>
			...
			<br/>
			<i><?php _e("The image you have picked will be scaled to this height.", "suf_theme"); ?></i>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id('icon_width'); ?>"><?php _e('Set the width for the category icon:', 'suf_theme'); ?></label>
			...
			<br/>
			<i><?php _e("The image you have picked will be scaled to this width.", "suf_theme"); ?></i>
		</p>
	</div>
</div>
<?php
